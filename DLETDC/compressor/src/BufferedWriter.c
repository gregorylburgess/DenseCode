...
/*------------------------------------------------------------------
 Adds n bytes to the buffer bW starting from the pointer orig. It
 also adds a '\0' character after those "n" bytes.
 If the buffer becomes full, the whole buffer is output to disk.
------------------------------------------------------------------*/
long writeBWPlusZero(BufferWrite bW, unsigned char *orig, long n) {
	byte *dst;
	register int i;

	if (bW->posInBuffer < WRITER_BUFFER_SIZE - (n+1)) {
